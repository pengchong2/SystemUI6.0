...
public class CallbackHandlerTest extends AndroidTestCase {
    ...
    public void testSignalCallback_setMobileDataIndicators() {
        ...
        int dataActivity = R.drawable.ct_stat_sys_signal_inout;
        int primarySimIcon = R.drawable.stat_sys_data_primary_simcard;
        /// @}
        mHandler.setMobileDataIndicators(status, qs, type, networkType, qsType, in, out,
            /// M: Add for CT6M. add activity icon @{
            dataActivity,
            primarySimIcon,
            /// @}
        ...
        ///M: Add for interface modify
        /// M: Add for CT6M. add activity icon @{
        ArgumentCaptor<Integer> dataActivityArg = ArgumentCaptor.forClass(Integer.class);
        ArgumentCaptor<Integer> primarySimIconArg = ArgumentCaptor.forClass(Integer.class);
        /// @}
        Mockito.verify(mSignalCallback).setMobileDataIndicators(statusArg.capture(),
                qsArg.capture(), typeIconArg.capture(), netTypeIconArg.capture(),
                qsTypeIconArg.capture(),inArg.capture(), outArg.capture(),
                /// M: Add for CT6M. add activity icon @{
                dataActivityArg.capture(),
                primarySimIconArg.capture(),
                /// @}
        ...
        assertEquals(out, (boolean) outArg.getValue());
        /// M: Add for CT6M. add activity icon @{
        assertEquals(dataActivity, (int) dataActivityArg.getValue());
        assertEquals(primarySimIcon, (int) primarySimIconArg.getValue());
        /// @}
