...
//			+ BUTTON_DELIMITER + PowerButton.BUTTON_WIFIAP + BUTTON_DELIMITER
//			+ PowerButton.BUTTON_WIFI + BUTTON_DELIMITER
//			+ PowerButton.BUTTON_MOBILEDATA + BUTTON_DELIMITER
//			+ PowerButton.BUTTON_AIRPLANE
//			// + BUTTON_DELIMITER + PowerButton.BUTTON_OFFSCREEN
//			+ BUTTON_DELIMITER + PowerButton.BUTTON_CARSETTING
//			+ BUTTON_DELIMITER + PowerButton.BUTTON_SETTINGS;

	private static final String BUTTONS_DEFAULT =
			PowerButton.BUTTON_MUTE+ BUTTON_DELIMITER
			+PowerButton.BUTTON_OFFSCREEN+ BUTTON_DELIMITER
			+ PowerButton.BUTTON_WIFI + BUTTON_DELIMITER
			+ PowerButton.BUTTON_MOBILEDATA + BUTTON_DELIMITER
	...
	// + BUTTON_DELIMITER + PowerButton.BUTTON_MOBILEDATA1
	// + BUTTON_DELIMITER + PowerButton.BUTTON_AIRPLANE1
	// + BUTTON_DELIMITER + PowerButton.BUTTON_OFFSCREEN1
	// + BUTTON_DELIMITER + PowerButton.BUTTON_RESET1
	// + BUTTON_DELIMITER + PowerButton.BUTTON_BLUETOOTH
	// + BUTTON_DELIMITER + PowerButton.BUTTON_LOCATION
	// + BUTTON_DELIMITER + PowerButton.BUTTON_SETTINGS;
	...
	static {
		...
		sPossibleButtons.put(PowerButton.BUTTON_RESET,
				ResetButton.class);
		sPossibleButtons.put(PowerButton.BUTTON_SETTINGS,
				SettingsButton.class);
		sPossibleButtons.put(PowerButton.BUTTON_LOCATION,
				LocationButton.class);
		sPossibleButtons.put(PowerButton.BUTTON_BLUETOOTH,
				BluetoothButton.class);
		sPossibleButtons.put(PowerButton.BUTTON_OFFSCREEN,
				OffScreenButton.class);
		sPossibleButtons.put(PowerButton.BUTTON_MOBILEDATA1,
				MobileDataButton.class);
		sPossibleButtons.put(PowerButton.BUTTON_AIRPLANE1,
				AirplaneModeButton.class);
		sPossibleButtons.put(PowerButton.BUTTON_OFFSCREEN1,
				OffScreenButton.class);
		sPossibleButtons.put(PowerButton.BUTTON_RESET1,
				ResetButton.class);
		sPossibleButtons.put(PowerButton.BUTTON_CARSETTING,
				CarSettingButton.class);
		sPossibleButtons.put(PowerButton.BUTTON_WALLPICK,
				WallPickButton.class);
		sPossibleButtons.put(PowerButton.BUTTON_SYSTEM_UPDATE,
				SystemUpdateButton.class);
		sPossibleButtons.put(PowerButton.BUTTON_SOUNDEFFECT,
				SoundEffectButton.class);
		sPossibleButtons.put(PowerButton.BUTTON_MUTE,
				MuteButton.class);
	...
	public void destroyWidget() {
		Flog.i(TAG, "Clearing any old widget stuffs");
	...
	public void setupWidget() {
		...
		String buttons = null /*
							 ...
							 */;
		if (buttons == null) {
			Flog.i(TAG, "Default buttons being loaded");
		...
		for (String button : buttons.split("\\|")) {
			if (loadButton(button)) {
				Log.d("yaoyao", "setupWidget: "+button);
				Log.d("pc","buttonname = "+button);
				mButtonNames.add(button);
			} else {
				Flog.d(TAG, "Error setting up button: " + button);
			}
		}
	...
	private void recreateButtonLayout() {
		...
		onelinearLayout.setWeightSum(7);
		...
		/*
		 * onelinearLayout.setBackgroundResource(R.drawable.systemui_powerbutton_bg
		 * );
		 * towlinearLayout.setBackgroundResource(R.drawable.systemui_powerbutton_bg
		 * );
		 */

		// XYL
		//add by pengchong
	//onelinearLayout.setBackground(
	//			SkinResource.getSkinDrawableByName("systemui_powerbutton_bg"));
	//	towlinearLayout.setBackground(
	//			SkinResource.getSkinDrawableByName("systemui_powerbutton_bg"));
		// 动态设置小圆点
		...
		for (int i = 0; i < mButtonNames.size(); i++) {
			...
			if (pb != null) {
				...
				//lp.rightMargin = 60;
				lp.weight = 1;
				//if(!"toggleSoundeffect".equals(pb.mType)){
					buttonView.setLayoutParams(lp);
				//}

				pb.setupButton(buttonView); // pb.setupButton(buttonView);
				if (i < 7) {
					Log.d("yaoyao", "recreateButtonLayout: onelinearLayout.addView(buttonView);");
				...
				Log.d("yaoyao", "recreateButtonLayout: pb=null");
			}
		}

		vlist.add(onelinearLayout);
//		 vlist.add(towlinearLayout);
		...
		for (int i = 0; i < mTips.length; i++) {
			...
			mTips[i] = imageView;
			if (i == 0) {
				Log.d(TAG, "recreateButtonLayout: R.drawable.page_indicator_focused?null");
				mTips[i].setBackground(SkinResource.getSkinDrawableByName("page_indicator_focused"));
			} else {
				Log.d(TAG, "recreateButtonLayout: R.drawable.page_indicator_unfocused?null");
	...
	class GuidePageChangeListener implements OnPageChangeListener {
		...
		public void onPageSelected(int arg0) {
			for (int i = 0; i < vlist.size(); i++) {
				...
				Flog.d("pwget", " vlist.size() = " + vlist.size()
						+ " mTips.length = " + mTips.length);
				mTips[arg0]
	...
	private IntentFilter getMergedBroadcastIntentFilter() {
		...
		Flog.d(TAG, "PowerWidget getMergedBroadcastIntentFilter");
	...
	private class WidgetBroadcastReceiver extends BroadcastReceiver {
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(Intent.ACTION_CONFIGURATION_CHANGED)) {
				updateButtonLayoutWidth();
				recreateButtonLayout();
			}else if(ConstantUtils.MUTE_ACTION.equals(action)){
				for (PowerButton button : mButtons.values()) {
	...
	private class WidgetSettingsObserver extends ContentObserver {
		...
		public void onChange(boolean selfChange, Uri uri) {
			 ...
			 * WIDGET_BUTTONS))) { setupWidget(); // now check if we change
			 * visibility } else
			 * if(uri.equals(Settings.System.getUriFor(Settings
			 * .System.EXPANDED_VIEW_WIDGET))) { updateVisibility(); // now
			 * check for scrollbar hiding } else
			 * if(uri.equals(Settings.System.getUriFor
			 * (Settings.System.EXPANDED_HIDE_SCROLLBAR))) { updateScrollbar();
			 * }
			 */
			...
			/*
			 * || uri.equals(Settings.Sy memoryrycle
			 * getBroadcastIntentFilterstem
