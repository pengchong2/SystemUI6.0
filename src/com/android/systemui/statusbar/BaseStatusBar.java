...
public abstract class BaseStatusBar extends SystemUI implements
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            } else if (action.equals("com.flyaudio.killVideoProcess")) {
                if (SkinResource.getSkinContext().getResources().getBoolean(
                        SkinResource.getSkinResourceId("isShowWaringDialog","bool"))) {
                    //行车中不允许切换app
                    FlyAudioProcess.getInstance().flyBrakeCar(intent,context);
                }
            }else if(action.equals("cn.flyaudio.systemui.action.recents")){
                Flog.d(TAG, "systemui recents action");
                toggleRecentApps();
            }
        }
    };
    ...
    public void start() {
        ...
        /*flyaudio */
        FlyAudioProcess.getInstance().initFlyCarStatus(mContext);
        ...
        /*flyaudio*/
        filter.addAction("com.flyaudio.killVideoProcess");
        filter.addAction("cn.flyaudio.systemui.action.recents");
        /*end*/
    ...
    private void bindGuts(ExpandableNotificationRow row) {
        ...
        DateTimeView dtv=((DateTimeView) row.findViewById(R.id.timestamp));
        dtv.setTime(sbn.getPostTime());
        dtv.setTextColor(SkinResource.getSkinColorByName("notification_guts_text_color"));
        TextView tv1=((TextView) row.findViewById(R.id.pkgname));
        tv1.setText(appname);
        tv1.setTextColor(SkinResource.getSkinColorByName("notification_guts_title_color"));
    ...
    protected NotificationData.Entry createNotificationViews(StatusBarNotification sbn) {
     ...
     // notification
 		if (FlyAudioProcess.getInstance().isNeedNotification) {

 			if (entry.notification.getPackageName().equals("com.android.phone")
 					|| entry.notification.getPackageName().equals(
 							"com.android.mms")) {
 				Log.d("phonenotify", "id phone || mms notify");
 				return null;
 			}
 		}

        if (FlyAudioProcess.getInstance().baiduOSDisPlayNotification(entry)) return null;

        if ("cn.flyaudio.flyaudiospeechservice".equals(entry.notification
                .getPackageName())) {
            return null;
        }

        Log.d("systemui", "createNotificationViews: notification:"+entry.notification
                .getPackageName());
        // end
        return entry;
    }
...
//        	interrupt = interrupt && !mDreamManager.isDreaming();
        	//framework底下已经去掉屏保的服务,所以会报空指针，因为屏保影响开关机
