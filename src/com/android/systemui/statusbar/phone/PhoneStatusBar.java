...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode,
    ...
    private List<FlyaudioServices> flyaudioServices;
    ...
    private static boolean roateSeekBarVoiceIV = false;
    ...
    private ImageView seekbar_voice_iv;
    private FlyNavigator mFlyNavigator;
    private SeekBar mSoundSeekBar;
    private TextView tv_rely_back;
    private TextView tv_rely_menu;
    private TextView tv_rely_home;
    private TextView tv_rely_music_up;
    private TextView tv_rely_music_next;
    private ImageView rely_music_up;
    private ImageView rely_music_next;
    private LinearLayout bottom_button_bg;

    private ImageButton mVolumeDecImgBtn,mVolumeIncImgBtn,mBrightnessDecImgBtn,mBrightnessIncImgBtn;
    private SeekBar mVolumeSbr,mBrightnessSbr;
    private TextView mVolumeTv,mBrightnessTv;
    //滑动音量进度条的时候，如果滑动到了0，需要将静音的图标置为选中状态
    private boolean ismute = false;
    //底部导航栏是否被清空了的标志
    private boolean isRemoveNavi = false;

    private static int currScreenBrightness = 0;
    private static boolean brightnessFromUser = false;
    private static int maxbrightness;
    private static int currprogress = 0;
    private static boolean VolumeFromUser = false;
    private int currVolumeStatus = -1;
    private int maxVolumeStatus;
    private MyHandler myHandler = new MyHandler();

    private static final int MG_SEEKBAR_VOLUME=5;
    private static final int MG_SEEKBAR_BRIGHTNESS=6;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MG_SEEKBAR_BRIGHTNESS:
                    ...
                    Log.d(TAG,"brightnessFromUser = "+brightnessFromUser);
                    break;
    ...
    private Runnable incScreenBrightness = new Runnable() {
        public void run() {
            if (myHandler != null && mBrightnessSbr != null) {
                myHandler.postDelayed(this, 160);
                if (mBrightnessSbr.getProgress() < mBrightnessSbr.getMax()) {
                    FlySystemManager.getInstance().incScreenBrightness(2);
                    Log.d(TAG," FlySystemManager.getInstance().incScreenBrightness ");
                } else {
    ...
    private Runnable decScreenBrightness = new Runnable() {
        public void run() {
            if (myHandler != null && mBrightnessSbr != null) {
                myHandler.postDelayed(this, 160);
                if (mBrightnessSbr.getProgress() > 1) {
                        FlySystemManager.getInstance().decScreenBrightness(2);
                        Log.d(TAG," FlySystemManager.getInstance().decScreenBrightness ");
                } else {
                    myHandler.removeCallbacks(this);
                }
            }
    ...
    private Runnable decVolume = new Runnable() {
        public void run() {
            if (myHandler != null && mVolumeSbr != null) {
                myHandler.postDelayed(this, 100);
                if (mVolumeSbr.getProgress() > 0) {
    ...
    private Runnable incVolume = new Runnable() {
        public void run() {
            if (myHandler != null && mVolumeSbr != null) {
                myHandler.postDelayed(this, 100);
                if (mVolumeSbr.getProgress() < mVolumeSbr.getMax()) {
    ...
    View.OnTouchListener l1 = new View.OnTouchListener() {
        @Override
        public boolean onTouch(View view, MotionEvent motionEvent) {
            switch (motionEvent.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (myHandler != null && decVolume != null) {
                        myHandler.removeCallbacks(decVolume);
                    }
                    if (myHandler != null && incVolume != null) {
                        myHandler.removeCallbacks(incVolume);
                    }
                    break;
    ...
    View.OnTouchListener l2 = new View.OnTouchListener() {
        @Override
        public boolean onTouch(View view, MotionEvent motionEvent) {
            switch (motionEvent.getAction()) {
                case MotionEvent.ACTION_UP:

                    if (myHandler != null && decScreenBrightness != null) {
                        myHandler.removeCallbacks(decScreenBrightness);
                    }
                    if (myHandler != null && incScreenBrightness != null) {
                        myHandler.removeCallbacks(incScreenBrightness);
                    }
                    break;
                case MotionEvent.ACTION_DOWN:
    ...
    //add by pengchong left naviwindowmanager
   private WindowManager mFlyLeftNaviWindowManger = null;
   private  View mFlyLeftNaviview = null; //左侧选择栏
   private View mFlyItemLayout = null; //底部导航栏
    ...
    public void onVolumeStatus(int i, int i2) {
        Log.d("pc","onVolumeStatus i = "+i+" i2 = "+i2);
        if (mVolumeSbr == null){
            return;
        }
        if (maxVolumeStatus != i) {
            maxVolumeStatus = i;
            mVolumeSbr.setMax(maxVolumeStatus);
            mVolumeSbr.setProgress(i2);
        }
...
//        if (mute != null) {
//            if (currVolumeStatus == 0) {
//                mute.setBackground(SkinResource.getSkinDrawableByName("qs_mute_d"));
//            } else {
//                mute.setBackground(SkinResource.getSkinDrawableByName("qs_mute_u"));
//            }
//        }
    }
    ...
    void onScreenBrightness(int i) {
        Log.d(TAG,"onScreenBrightness i = "+i+" currScreenBrightness = "+currScreenBrightness+" brightnessFromUser ="+brightnessFromUser);
        if (mBrightnessSbr == null){
            return;
        }
        Log.d(TAG,"maxbrightness = "+maxbrightness+" flymax = "+FlySystemManager.getInstance().getMaxScreenBrightness()+" flymin = "+FlySystemManager.getInstance().getMinScreenBrightness());
        ...
        //    mBrightnessSbr.setMax(FlySystemManager.getInstance().getMaxScreenBrightness() - FlySystemManager.getInstance().getMinScreenBrightness());
            mBrightnessSbr.setMax(FlySystemManager.getInstance().getMaxScreenBrightness());
            mBrightnessSbr.setProgress(i);
        }
    ...
    protected PhoneStatusBarView makeStatusBarView() {
        FlySDKManager.getInstance().initialize(mContext, new InitListener() {
            @Override
            public void onError() {
                Log.d(TAG,"FlySDKManager init error");
            }

            @Override
            public void onSucceed() {
                Log.d(TAG,"FlySDKManager init succeed");
        ...
        mFlyNavigator = FlyNavigator.getInstance(mContext);
        ...
        //add navilayout in statusbar
//       View naviview =  View.inflate(mContext,R.layout.flyaudio_navigationbar_layout,null);
//       FlyaudioSystemUI.getInstance().initNaviView(naviview);
//       mStatusBarView.addView(naviview);
        ...
        /*flyaudio 初始化状态栏蓝牙图标*/
        FlyAudioProcess.getInstance().setStatusBarBtIcon(mStatusBarWindow);
        /*end*/


        bottom_button_bg = (LinearLayout) mStatusBarWindow
                .findViewById(R.id.bottom_button_bg);

        bottom_button_bg.setBackground(
                SkinResource.getSkinDrawableByName("bottom_switch_buttom_bg"));

        int flags = SkinResource.getSkinContext().getApplicationInfo().flags;

        SkinResource.getSkinContext().getApplicationInfo().flags=
                SkinResource.getSkinContext().getApplicationInfo().flags |=ApplicationInfo.FLAG_SUPPORTS_RTL;

          /*加载seekbar*/
        View bottom_seekbar_root = SkinResource.getSkinLayoutViewByName("bottom_seekbarview");


        View bottom_button = SkinResource.getSkinLayoutViewByName("bottom_button");

        View bottom_pointer = SkinResource.getSkinLayoutViewByName("bottom_pointer");

        bottom_button_bg.addView(bottom_seekbar_root);

       bottom_button_bg.addView(bottom_button);

       bottom_button_bg.addView(bottom_pointer);


       mVolumeTv = (TextView)mStatusBarWindow.findViewById(R.id.qs_seekbar_volume_title);
       mBrightnessTv = (TextView)mStatusBarWindow.findViewById(R.id.qs_seekbar_brightness_title);
       mVolumeDecImgBtn = (ImageButton)mStatusBarWindow.findViewById(R.id.qs_seekbar_volume_dec) ;
       mVolumeDecImgBtn.setOnClickListener(new View.OnClickListener(){
           ...
           public void onClick(View v) {
               ...
               if(currVolumeStatus>0){
                   currVolumeStatus--;
                   mVolumeSbr.setProgress(currVolumeStatus);
               }
           }
       });


       mVolumeDecImgBtn.setOnLongClickListener(new View.OnLongClickListener(){
           ...
           public boolean onLongClick(View v) {
               ...
               if (mVolumeSbr != null && decVolume != null) {
                   mVolumeSbr.post(decVolume);
               }
               return false;
           }
       });

       mVolumeIncImgBtn = (ImageButton)mStatusBarWindow.findViewById(R.id.qs_seekbar_volume_inc);

       mVolumeIncImgBtn.setOnClickListener(new View.OnClickListener(){

           @Override
           public void onClick(View v) {
               FlySystemManager.getInstance().incVolume(1);
               if(currVolumeStatus<maxVolumeStatus){
                   currVolumeStatus++;
                   mVolumeSbr.setProgress(currVolumeStatus);
               }
           }
       ...
       mVolumeIncImgBtn.setOnLongClickListener(new View.OnLongClickListener(){

           @Override
           public boolean onLongClick(View v) {
               if (mVolumeSbr == null) return true;
               if (mVolumeSbr.getProgress() == mVolumeSbr.getMax()) {
                   return true;
               }
               if (myHandler != null && incVolume != null) {
                   myHandler.post(incVolume);
               }
               return false;
           }
       });

        mVolumeIncImgBtn.setOnTouchListener(l1);
        mVolumeDecImgBtn.setOnTouchListener(l1);


       mVolumeSbr = (SeekBar)mStatusBarWindow.findViewById(R.id.id_seekBar_volume) ;

       mVolumeSbr.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
           @Override
           public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
               Log.d(TAG,"progress = "+progress+" ismute = "+ismute);
               if(progress == 0){
                   mContext.sendBroadcast(new Intent(ConstantUtils.MUTE_ACTION).putExtra("ismute",true));
                   ismute = true;
               }else if(ismute && progress>0){
                   ismute = false;
                   mContext.sendBroadcast(new Intent(ConstantUtils.MUTE_ACTION).putExtra("ismute",false));
               }
               isfromUser = !fromUser;

               Flog.d(TAG, "SeekBar onProgressChanged: "
                       + isfromUser);
               if (fromUser) {
                   VolumeFromUser = true;
                   String maxsoundStr = SystemProperties.get(
                           "fly.HAL.MaxVolume", "-1");
                   Flog.d(TAG,
                           "mSeekBarHandle maxsoundStr ============== "
                                   + maxsoundStr.toString());
                   if (INIT_PRO_SEEKBAR_MAX) {
                       INIT_PRO_SEEKBAR_MAX = false;
                       Log.d(TAG,
                               "***onProgressChanged INIT_SEEKBAR_MAX ONCE***");
                       if (!maxsoundStr.equals("-1")) {
                           int maxsound = Integer
                                   .parseInt(maxsoundStr);
                           mSoundSeekBar.setMax(maxsound);
                       } else {
                           // Log.d(TAG, "maxsound = " + maxsound);
                           mSoundSeekBar.setMax(40);
                       }
                   }
                   // isfromUser = false;
                            /*mFlyNavigator.new AdapterCenter()
                                    .sendControlVolum(progress);*/
                   FlySystemManager.getInstance().setVolume(progress);
           ...
           public void onStartTrackingTouch(SeekBar seekBar) {
               mSeekBarHandle.removeMessages(0);
               isfromUser = false;
           }

           @Override
           public void onStopTrackingTouch(SeekBar seekBar) {
               mSeekBarHandle.sendEmptyMessageDelayed(0, 200);
               Flog.d(TAG,
                       "*** onStopTrackingTouch step end : "
                               + isfromUser);
           }
       });


       mBrightnessDecImgBtn = (ImageButton)mStatusBarWindow.findViewById(R.id.qs_seekbar_brightness_dec) ;
       mBrightnessDecImgBtn.setOnClickListener(new View.OnClickListener(){

           @Override
           public void onClick(View v) {
               if (currScreenBrightness > 0) {
                 currScreenBrightness --;
                 Log.d(TAG,"mBrightnessDecImgBtn decrese brightness currScreenBrightness = "+currScreenBrightness);
                 FlySystemManager.getInstance().setScreenBrightness(currScreenBrightness );
                 mBrightnessSbr.setProgress(currScreenBrightness);
               }
           }
       });

       mBrightnessDecImgBtn.setOnLongClickListener(new View.OnLongClickListener(){

           @Override
           public boolean onLongClick(View v) {

               if (mBrightnessSbr == null) return true;
               if (mBrightnessSbr.getProgress() == 0) {
                   return true;
               }
               if (myHandler != null) {
                   myHandler.post(decScreenBrightness);
               }
       ...
       mBrightnessIncImgBtn = (ImageButton)mStatusBarWindow.findViewById(R.id.qs_seekbar_brightness_inc) ;

        mBrightnessIncImgBtn.setOnClickListener(new View.OnClickListener(){

            @Override
            public void onClick(View v) {
                if (currScreenBrightness < maxbrightness) {
                    currScreenBrightness ++;
                    Log.d(TAG,"mBrightnessIncImgBtn increse brightness currScreenBrightness = "+currScreenBrightness);
                    FlySystemManager.getInstance().setScreenBrightness(currScreenBrightness);
                    mBrightnessSbr.setProgress(currScreenBrightness);
                }
            }
        ...
        mBrightnessIncImgBtn.setOnLongClickListener(new View.OnLongClickListener(){
            ...
            public boolean onLongClick(View v) {
                ...
                if (myHandler != null) {
                    myHandler.post(incScreenBrightness);
                }
        ...
        mBrightnessIncImgBtn.setOnTouchListener(l2);
        mBrightnessDecImgBtn.setOnTouchListener(l2);

       mBrightnessSbr = (SeekBar)mStatusBarWindow.findViewById(R.id.id_seekBar_brightness) ;

       mBrightnessSbr.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
           @Override
           public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {

               if (fromUser) {
                   brightnessFromUser = true;
                   Log.d(TAG,"brightness setScreenBrightness progress = "+progress);
        ...
        FlySystemManager.getInstance().setSystemListener(new SystemListener() {
            ...
            public void onVolumeStatus(int i, int i1, int i2) {
               ...
               PhoneStatusBar.this.onVolumeStatus(i,i2);
            }
            ...
            public void onScreenBrightness(int i) {
             ...
             PhoneStatusBar.this.onScreenBrightness(i);
            ...
            public void onCurrentPageChanged(int i) {
                FlyaudioSystemUI.getInstance().setItemChecked(i);
            }
        ...
        //mSoundSeekBar
        mSoundSeekBar = (SeekBar)
                mStatusBarWindow.findViewById(SkinResource.getSkinResourceId("id_seekBar", "id"));
        ...
        mSoundSeekBar .setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
                    ...
                    public void onStopTrackingTouch(SeekBar seekBar) {
                        // Log.d("myseekbar",
                        // "*** onStopTrackingTouch step 1 : " + isfromUser);
                        // isfromUser = true;
                        mSeekBarHandle.sendEmptyMessageDelayed(0, 200);
                        Flog.d(TAG,
                                "*** onStopTrackingTouch step end : "
        ...
        ImageView ic_systemui_voice = (ImageView) mStatusBarWindow
                .findViewById(SkinResource.getSkinResourceId("ic_systemui_voice", "id"));
        ic_systemui_voice.setImageDrawable(
                SkinResource.getSkinDrawableByName("bottom_seekbar_voice_bg"));

        tv_sound_size = (TextView) mStatusBarWindow
                .findViewById(SkinResource.getSkinResourceId("tv_sound_size", "id"));


       /* View bottom_seekbar_root = LayoutInflater.from(SkinResource.getSkinContext()).inflate(
                SkinResource.getSkinLayoutIdByName("bottom_seekbar_root"), null);*/

        seekbar_voice_iv = (ImageView)
                bottom_seekbar_root.findViewById(
                        SkinResource.getSkinResourceId("ic_systemui_voice", "id"));


        FrameLayout backLayout, homeLayout, preLayout,mdateFrameLayout,menuLayout,nextLayout;
        ImageView rely_back,rely_menu,rely_home;

         backLayout = (FrameLayout) mStatusBarWindow
                 .findViewById(SkinResource.getSkinResourceId("rback", "id"));
        tv_rely_back = (TextView) mStatusBarWindow
                .findViewById(SkinResource.getSkinResourceId("tv_rely_back", "id"));
        rely_back = (ImageView) mStatusBarWindow.
                findViewById(SkinResource.getSkinResourceId("rely_back", "id"));


        menuLayout = (FrameLayout) mStatusBarWindow
                .findViewById(SkinResource.getSkinResourceId("rmenu", "id"));
        rely_menu = (ImageView) mStatusBarWindow.
                findViewById(SkinResource.getSkinResourceId("rely_menu", "id"));
        tv_rely_menu = (TextView) mStatusBarWindow
                .findViewById(SkinResource.getSkinResourceId("tv_rely_menu", "id"));


        homeLayout = (FrameLayout) mStatusBarWindow
                .findViewById(SkinResource.getSkinResourceId("rhome", "id"));
        rely_home = (ImageView) mStatusBarWindow.
                findViewById(SkinResource.getSkinResourceId("rely_home", "id"));
        tv_rely_home = (TextView) mStatusBarWindow
                .findViewById(SkinResource.getSkinResourceId("tv_rely_home", "id"));


        preLayout = (FrameLayout) mStatusBarWindow
                .findViewById(SkinResource.getSkinResourceId("pre_music", "id"));
        rely_music_up = (ImageView) mStatusBarWindow
                .findViewById(SkinResource.getSkinResourceId("rely_music_up", "id"));
        tv_rely_music_up = (TextView) mStatusBarWindow
                .findViewById(SkinResource.getSkinResourceId("tv_rely_music_up", "id"));


        nextLayout = (FrameLayout) mStatusBarWindow
                .findViewById(SkinResource.getSkinResourceId("next_music", "id"));
        rely_music_next = (ImageView) mStatusBarWindow
                .findViewById(SkinResource.getSkinResourceId("rely_music_next", "id"));
        tv_rely_music_next = (TextView) mStatusBarWindow
                .findViewById(SkinResource.getSkinResourceId("tv_rely_music_next", "id"));

        setBottomButtonText(
                tv_rely_back, tv_rely_menu, tv_rely_home, tv_rely_music_up, tv_rely_music_next,
                rely_music_up, rely_music_next);

        if (SkinResource.getSkinBooleanFromSkin("is_pakistan",false)) {
            rely_back.setImageDrawable(SkinResource.getSkinDrawableByName("bottom_rely_menu_selector"));
            rely_home.setImageDrawable(SkinResource.getSkinDrawableByName("bottom_rely_gps_selector"));
            rely_menu.setImageDrawable(SkinResource.getSkinDrawableByName("bottom_rely_audio_selector"));
        }else {
            rely_back.setImageDrawable(SkinResource.getSkinDrawableByName("bottom_rely_back_selector"));
            rely_home.setImageDrawable(SkinResource.getSkinDrawableByName("bottom_rely_home_selector"));
            rely_menu.setImageDrawable(SkinResource.getSkinDrawableByName("bottom_rely_menu_selector"));
        }
        rely_back.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                animateCollapsePanels();
                if (SkinResource.getSkinBooleanFromSkin("is_pakistan",false)) {
        ...
        rely_home.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                animateCollapsePanels();
                if (SkinResource.getSkinBooleanFromSkin("is_pakistan",false)) {
                    ...
                    Log.d("wfllog", "(byte)(pageID&0xff) : "
                            ...
                            + (byte) ((pageID & 0xff00) >> 8));
                    mFlyNavigator.new AdapterCenter().setZControl(id, (byte) 0xFE,
                            seekbar_increase);
                }else {
        ...
        rely_menu.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                animateCollapsePanels();
                if (SkinResource.getSkinBooleanFromSkin("is_pakistan",false)) {
        ...
        rely_music_next.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (SkinResource.getSkinBooleanFromSkin("isshow_gpsandsound")) {
                    int pageID = 257;
                    byte[] seekbar_increase = { 0x10, (byte) 0x82,(byte) ((pageID & 0xff00) >> 8),(byte) (pageID & 0xff)};
                    int id = 0xFC0001;
                    Log.d("wfllog", "(byte)(pageID&0xff) : "
                    ...
                    mFlyNavigator.new AdapterCenter().setZControl(id, (byte) 0xFE,
                            seekbar_increase);
                    animateCollapsePanels();
                }else if(SkinResource.getSkinBooleanFromSkin("isshow_music_control")){
                    if (isARorIW()) {
                        byte[] music_pre = { 0x05, 0x01, 0x01 };
                        int id = 0xFC0001;
                        mFlyNavigator.new AdapterCenter().setZControl(id, (byte) 0xFE,
                                music_pre);
                    }else {
                        byte[] music_next = { 0x05, 0x01, 0x00 };// 下一首、下一曲、下一个
                        int id = 0xFC0001;
                        mFlyNavigator.new AdapterCenter().setZControl(id, (byte) 0xFE,
                                music_next);
                    }
        ...
        rely_music_up.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (SkinResource.getSkinBooleanFromSkin("isshow_gpsandsound")) {
                    int pageID = 4368;
                    ...
                    if (isARorIW()) {
                        ...
                        byte[] music_pre = { 0x05, 0x01, 0x01 };// 上一首、上一曲、上一个
                        int id = 0xFC0001;
                        mFlyNavigator.new AdapterCenter().setZControl(id, (byte) 0xFE,
                                music_pre);
                    }
        ...
        setButtomButtonOntouchEventListener(rely_back, rely_menu, rely_home);
...
//                Log.d(TAG,"isSupported = "+MultiWindowProxy.isSupported());
//                if(MultiWindowProxy.isSupported()) {
//
...
//        }
        ...
        /*flyaduio 监听蓝牙状态 改变状态*/
        filter.addAction("com.android.action.bluestate");
        filter.addAction("com.android.systemui.screenoff");
        filter.addAction("com.flyaudio.power");
        filter.addAction(Intent.ACTION_CONFIGURATION_CHANGED);
        filter.addAction(Intent.ACTION_BOOT_COMPLETED);
        filter.addAction("action.flyaudio.colortheme");
        filter.addAction("com.android.flyaudioui.volume");
        /*end*/
    ...
    private void setBottomButtonText(TextView tv_rely_back, TextView tv_rely_menu, TextView tv_rely_home, TextView tv_rely_music_up, TextView tv_rely_music_next, ImageView rely_music_up, ImageView rely_music_next) {
    /*以色列 上下一曲*/
    //isARorIW()
        if (SkinResource.getSkinBooleanFromSkin("isshow_music_control")) {
            rely_music_up.setImageDrawable(SkinResource.getSkinDrawableByName("bottom_rely_music_up_selector"));
            rely_music_next.setImageDrawable(SkinResource.getSkinDrawableByName("bottom_rely_music_next_selector"));
            tv_rely_music_up.setText(SkinResource.getSkinStringByName("rely_music_up"));
            tv_rely_music_next.setText(SkinResource.getSkinStringByName("rely_music_next"));
        }else if (SkinResource.getSkinBooleanFromSkin("isshow_gpsandsound")){
        /*国内 导航/音量*/
            rely_music_up.setImageDrawable(SkinResource.getSkinDrawableByName("bottom_rely_gps_selector"));
            rely_music_next.setImageDrawable(SkinResource.getSkinDrawableByName("bottom_rely_audio_selector"));
            tv_rely_music_up.setText(SkinResource.getSkinStringByName("rely_gps"));
            tv_rely_music_next.setText(SkinResource.getSkinStringByName("rely_audio"));
        }
        if (SkinResource.getSkinBooleanFromSkin("is_pakistan",false)) {
            tv_rely_back.setText(SkinResource.getSkinStringByName("rely_menu"));
            tv_rely_home.setText(SkinResource.getSkinStringByName("rely_gps"));
            tv_rely_menu.setText(SkinResource.getSkinStringByName("rely_audio"));
        }else {
            tv_rely_back.setText(SkinResource.getSkinStringByName("rely_back"));
            tv_rely_home.setText(SkinResource.getSkinStringByName("rely_home"));
            tv_rely_menu.setText(SkinResource.getSkinStringByName("rely_menu"));
        }
        bottom_button_bg.setBackground(
                SkinResource.getSkinDrawableByName("bottom_switch_buttom_bg"));
        seekbar_voice_iv.setImageDrawable(
                SkinResource.getSkinDrawableByName("bottom_seekbar_voice_bg"));
    }
    ...
    public int getStatusBarHeight() {
        ...
        Log.d("pc","mNaturalBarHeight = "+mNaturalBarHeight);
       // mNaturalBarHeight = 600;
    ...
    public void addNavigationBar() {
        ...
        //add flyaudio item layout
       // FlyaudioSystemUI.getInstance().initView(flyitemlayout);
        mFlyItemLayout = LayoutInflater.from(mContext).inflate(R.layout.flyaudio_systemui_item_layout,null);
        FlyaudioSystemUI.getInstance().initView(mFlyItemLayout,this);
        mWindowManager.addView(mFlyItemLayout,getNavigationBarLayoutParams());

        // avoid original navibar by pengchong
...
//        mWindowManager.addView(mNavigationBarView, getNavigationBarLayoutParams());
    ...
    public void removeNavigationBar(){
        if(mWindowManager!=null && mFlyItemLayout !=null && !isRemoveNavi){
           // mWindowManager.removeView(mFlyItemLayout);
            try{
                mWindowManager.removeViewImmediate(mFlyItemLayout);
                isRemoveNavi = true;
            }catch (Exception e){
    ...
    /**
     * 当点击地图和天气按钮的时候，会移除底部的导航栏，这个时候点击其他的按钮的时候，先判断
     * 下底部的导航栏是否存在，如果不存在的话，重新生成新的导航栏
     */
    public void addNavigationBarAgain(){
        if(mWindowManager != null && mFlyItemLayout!= null &&isRemoveNavi){
          //  mFlyItemLayout = LayoutInflater.from(mContext).inflate(R.layout.flyaudio_systemui_item_layout,null);
          //  FlyaudioSystemUI.getInstance().initViewAgain(mFlyItemLayout,this);
            try {
                mWindowManager.addView(mFlyItemLayout, getNavigationBarLayoutParams());
                isRemoveNavi = false;
            }catch (Exception e){
    ...
    /**
     * 添加systemui左侧的导航栏
     * 通过windowmanager的方式添加
     */
    private void addFlySystemUILeftNaviBar(){

        mFlyLeftNaviWindowManger = (WindowManager)mContext.getSystemService(Context.WINDOW_SERVICE);
        mFlyLeftNaviview =  View.inflate(mContext,R.layout.flyaudio_navigationbar_layout,null);
        mFlyLeftNaviview.setFitsSystemWindows(true);
        FlyaudioSystemUI.getInstance().initNaviView(mFlyLeftNaviview);
        mFlyLeftNaviWindowManger.addView(mFlyLeftNaviview,getLeftNavigationBarLayoutParams());

        Display display = mFlyLeftNaviWindowManger.getDefaultDisplay();
        Point p  = new Point();
        display.getRealSize(p);

        Log.d(TAG,"display width = "+p.x+"display height = "+p.y);

    }

    private WindowManager.LayoutParams getLeftNavigationBarLayoutParams() {

        WindowManager.LayoutParams lp = new WindowManager.LayoutParams(
                mContext.getResources().getDimensionPixelOffset(R.dimen.flyaudio_left_window_with), mContext.getResources().getDimensionPixelOffset(R.dimen.flyaudio_left_window_height),
        ...
        Log.d(TAG,"x = "+lp.x+" y = "+lp.y+" width = "+lp.width+" height = "+lp.height+"widht = "+dm.widthPixels+" height = "+dm.heightPixels);
        return lp;
    }

    private WindowManager.LayoutParams getLeftNavigationBarLayoutParams2() {
        WindowManager.LayoutParams lp = new WindowManager.LayoutParams(
                120, 120,
                2017,WindowManager.LayoutParams.FLAG_FULLSCREEN
                | WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN
                |WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS
                | WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE,
                PixelFormat.TRANSLUCENT);
        ...
        lp.x = 0;
        lp.y = 0;
        Log.d(TAG,"x = "+lp.x+" y = "+lp.y+" width = "+lp.width+" height = "+lp.height);
        return lp;
    }
    ...
    void makeExpandedVisible(boolean force) {
        Log.d("pc","makeExpandedVisible force = "+force);
      ...
      //flyaudio panel
        //设置系统属性，目前下拉已经打开，不要显示类似音量的弹窗
        SystemProperties.set("fly.persist.statusbar.show", "1");
        mFlyNavigator.new AdapterCenter().sendFlyAppManger(126, FlyConstant.ON);
       //end
    ...
    public void animateCollapseQuickSettings() {
        if (mState == StatusBarState.SHADE) {
            Log.d("pc","phoneStatusBar animateCollapseQuickSettings()");
    ...
    void makeExpandedInvisible() {
        if (SPEW) Log.d(TAG, "makeExpandedInvisible: mExpandedVisible=" + mExpandedVisible
                + " mExpandedVisible=" + mExpandedVisible);
        Log.d("pc","makeExpandedInvisible mExpandedVisible = "+mExpandedVisible);
        if (!mExpandedVisible || mStatusBarWindow == null) {
            return;
        }
	    // panel
        //设置系统属性，目前下拉已经关闭，可以显示类似音量的弹窗
        SystemProperties.set("fly.persist.statusbar.show", "0");
	    mFlyNavigator.new AdapterCenter().sendFlyAppManger(126, FlyConstant.OFF);
	    // end
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            else if (action.equals("action.flyaudio.colortheme")) {
                //updateColor(intent);
                if (mNotificationPanel!=null) {
                    mNotificationPanel.systemColorChange(intent);

                }
            }
            else if (action.equals("com.android.flyaudioui.volume")) {
                volumesValue = intent.getIntExtra("key_volume", 0);
                tv_sound_size.setText(volumesValue + "");
                if (isfromUser)
                    mSeekBarHandle.sendEmptyMessage(0);
            } else if (action.equals(Intent.ACTION_CONFIGURATION_CHANGED)) {
                mNotificationPanel.invalidate();
                mVolumeTv.setText(R.string.qs_seekbar_volume_text);
                mBrightnessTv.setText(R.string.qs_seekbar_brightness_text);
            } else if (action.equals(Intent.ACTION_BOOT_COMPLETED)) {

                String dayNight = SystemProperties.get("fly.android.micsw", "40");
                String keyValuePairs = "Set_Mic_SW_Gain=" + dayNight;
                AudioManager audioManager = (AudioManager) SystemUIApplication.applicationContext.getSystemService(AUDIO_SERVICE);
                Log.d("AudioSystem", ">> onReceive: keyValuePairs>" + keyValuePairs);
                audioManager.setParameters(keyValuePairs);
            }

            /*flyaudio */
            else if (intent.getAction().equals("com.android.action.bluestate")) {
                FlyAudioProcess.getInstance().refreshFlyaudioUI(intent);
            }
            /*通过点击开关 关闭屏幕时候收起下拉*/
            else if (action.equals("com.android.systemui.screenoff")) {
            ...
            /*监听power 关闭屏幕收起下拉*/
            else if (intent.getAction().equals("com.flyaudio.power")) {
                FlyAudioProcess.getInstance().screenOffCloseNoti(intent, mContext);
                Bundle bundle = intent.getBundleExtra(PowerBundle);

                if (bundle.getString(KeyBootState).equals("KEY_SCREEN_ON") && mContext != null) {
                    flyaudioServices = SystemUIApplication.flyaudioServices;
                    if (flyaudioServices != null) {
                        if (flyaudioServices.size() > 0) {
                            startFlyaudioServerIfAccOn();
                        }
                    }
    ...
    public void destroy() {
        ...
        if(mFlyLeftNaviWindowManger != null && mFlyLeftNaviview !=null){
            mFlyLeftNaviWindowManger.removeViewImmediate(mFlyLeftNaviview);
            mFlyLeftNaviview = null;
        }

        if(mFlyItemLayout != null){
            mWindowManager.removeViewImmediate(mFlyItemLayout);
            mFlyItemLayout = null;
        }
    ...
    /*检索当前语言*/
    private boolean isARorIW(){
        return "iw".equals(Locale.getDefault().getLanguage())
                || ("ar".equals(Locale.getDefault().getLanguage()));
    }

    private void updateFlyaudioView() {
        if (seekbar_voice_iv!=null) {
            if (isARorIW()) {
                Log.d("qwee", "updateFlyaudioView: 180f");
                roateSeekBarVoiceIV = true;
                seekbar_voice_iv.setRotation(180f);
            }else {
                Log.d("qwee", "updateFlyaudioView: roateSeekBarVoiceIV:"+roateSeekBarVoiceIV);
                /*if (roateSeekBarVoiceIV) {
                    seekbar_voice_iv.setRotation(180f);
                    roateSeekBarVoiceIV = false;
                }*/
    ...
    private static boolean isfromUser = true;
    private static boolean INIT_PRO_SEEKBAR_MAX = true;
    private static boolean INIT_HAD_SEEKBAR_MAX = true;
    private ImageView[] tips;
    int volumesValue = -1;
    int equalvolumesValue = volumesValue;
    Handler mSeekBarHandle = new Handler() {
        @Override
        public void handleMessage(Message msg) {
            String maxsoundStr = SystemProperties
                    .get("fly.HAL.MaxVolume", "-1");

            if (INIT_HAD_SEEKBAR_MAX) {
                INIT_HAD_SEEKBAR_MAX = false;
                if (!maxsoundStr.equals("-1")) {
                    int maxsound = Integer.parseInt(maxsoundStr);
                    mSoundSeekBar.setMax(maxsound);
                } else {
                    // Log.d(TAG, "maxsound = " + maxsound);
                    mSoundSeekBar.setMax(40);
            ...
            if (!(equalvolumesValue == volumesValue) && isfromUser) {
                mSoundSeekBar.setProgress(volumesValue);
                tv_sound_size.setText(volumesValue + "");
            }
            equalvolumesValue = volumesValue;
            isfromUser = true;
        }
    };


    private void setButtomButtonOntouchEventListener(ImageView rely_back,ImageView rely_menu,ImageView rely_home){
        rely_music_up.setOnTouchListener(new ImageView.OnTouchListener() {

            @Override
            public boolean onTouch(View view, MotionEvent event) {
                // TODO Auto-generated method stub
                if (event.getAction() == MotionEvent.ACTION_DOWN) {
                    tv_rely_music_up.setTextColor(SkinResource.getSkinColorByName("systemui_buttom_textcolor_d"));
                }else if (event.getAction() == MotionEvent.ACTION_UP || event.getAction() == MotionEvent.ACTION_CANCEL) {
                    tv_rely_music_up.setTextColor(SkinResource.getSkinColorByName("systemui_buttom_textcolor_u"));
                }
                return false;
            }
        });

        rely_music_next.setOnTouchListener(new ImageView.OnTouchListener() {

            @Override
            public boolean onTouch(View view, MotionEvent event) {
                // TODO Auto-generated method stub
                if (event.getAction() == MotionEvent.ACTION_DOWN) {
                    tv_rely_music_next.setTextColor(SkinResource.getSkinColorByName("systemui_buttom_textcolor_d"));
                }else if (event.getAction() == MotionEvent.ACTION_UP || event.getAction() == MotionEvent.ACTION_CANCEL) {
                    tv_rely_music_next.setTextColor(SkinResource.getSkinColorByName("systemui_buttom_textcolor_u"));
                }
                return false;
            }
        });

        rely_back.setOnTouchListener(new ImageView.OnTouchListener() {

            @Override
            public boolean onTouch(View view, MotionEvent event) {
                // TODO Auto-generated method stub
                if (event.getAction() == MotionEvent.ACTION_DOWN) {
                    tv_rely_back.setTextColor(SkinResource.getSkinColorByName("systemui_buttom_textcolor_d"));
                }else if (event.getAction() == MotionEvent.ACTION_UP || event.getAction() == MotionEvent.ACTION_CANCEL) {
                    tv_rely_back.setTextColor(SkinResource.getSkinColorByName("systemui_buttom_textcolor_u"));
                }
                return false;
            }
        });

        rely_menu.setOnTouchListener(new ImageView.OnTouchListener() {

            @Override
            public boolean onTouch(View view, MotionEvent event) {
                // TODO Auto-generated method stub
                if (event.getAction() == MotionEvent.ACTION_DOWN) {
                    tv_rely_menu.setTextColor(SkinResource.getSkinColorByName("systemui_buttom_textcolor_d"));
                }else if (event.getAction() == MotionEvent.ACTION_UP || event.getAction() == MotionEvent.ACTION_CANCEL) {
                    tv_rely_menu.setTextColor(SkinResource.getSkinColorByName("systemui_buttom_textcolor_u"));
                }
                return false;
            }
        });

        rely_home.setOnTouchListener(new ImageView.OnTouchListener() {

            @Override
            public boolean onTouch(View view, MotionEvent event) {
                // TODO Auto-generated method stub
                if (event.getAction() == MotionEvent.ACTION_DOWN) {
                    tv_rely_home.setTextColor(SkinResource.getSkinColorByName("systemui_buttom_textcolor_d"));
                }else if (event.getAction() == MotionEvent.ACTION_UP || event.getAction() == MotionEvent.ACTION_CANCEL) {
                    tv_rely_home.setTextColor(SkinResource.getSkinColorByName("systemui_buttom_textcolor_u"));
                }
                return false;
            }
        });
    ...
    public void startFlyaudioServerIfAccOn(){
        if (flyaudioServices.size()>0) {
            for (FlyaudioServices flyaudioService : flyaudioServices) {
                if (flyaudioService.isAcc_start()) {
                    if (flyaudioService.getClass_name() != null &&
                            !"".equals(flyaudioService.getClass_name())) {
                        Log.d(TAG, "startFlyaudioServerIfAccOn: +"+flyaudioService.getClass_name());
                    ...
                    if (!"".equals(flyaudioService.getAciton_name())
                        ...
                        Log.d(TAG, "startFlyaudioServerIfAccOn: ");
                        Intent intent = new Intent();
