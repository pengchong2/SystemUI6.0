...
import com.flyaudio.view.ChooseStateTextView;
import com.flyaudio.view.PowerWidget;
import com.flyaudio.view.lineView;
...
public class NotificationPanelView extends PanelView implements
    ...
    public static boolean mQsExpanded;
    ...
    private ChooseStateTextView switchView;
    private ChooseStateTextView notificationView;
    private Resources skinResources;
    private int rb_default;
    private int rb_selected;
    private FrameLayout switchAndNotificationRoot;
    private ChooseStateTextView or_tv;
    private View line;
    private lineView mlv;
    private int switch_notification_marginStart;
    private int color_header_textview_d;
    private int color_header_textview_u;
    ...
    protected void onFinishInflate() {
        ...
        mHeader.setBackgroundColor(SkinResource.getSkinColorByName("status_bar_header_view_background_color"));
        //add by pengchong
     ...
     //   mHeader.setClickable(false);
        ...
        skinResources = SkinResource.getSkinContext().getResources();
        mViewFlipper = (ViewFlipper) findViewById(R.id.flipper);

        /*flyaudio header*/
        switchView = (ChooseStateTextView) findViewById(R.id.status_tag);
        notificationView = (ChooseStateTextView) findViewById(R.id.notification_tag);

        switchAndNotificationRoot = (FrameLayout)findViewById(R.id.head_layout);
        or_tv = (ChooseStateTextView) findViewById(R.id.or);
        line = findViewById(R.id.line);
        mlv = (lineView) findViewById(R.id.line_tag);

        initSkinFlyHeaderView();
        //rg_button = (RadioGroup)findViewById(R.id.rg_status_bar);
      //  mViewFlipper.setDisplayedChild(1);
        mViewFlipper.setDisplayedChild(0);
        setRadioButtonTextSize(1);
        changeLineViewLayoutParams(true);

        /*end*/

        //setQsContainerBackground();
    ...
    public void systemColorChange(Intent intent){
        int colortheme = intent.getIntExtra("rgb", 0);
        if (colortheme == 0) {
            mQsContainer.setBackgroundColor(Color.BLACK);
        }else {
           // setQsContainerBackground();
        }
    }

    private void setQsContainerBackground() {
        String colorFromPro = SystemProperties.get(
                "persist.fly.colortheme", "-65536");
        boolean isColorSkin = SkinResource.getSkinStringByName("isColorSkin").equals("isColorSkin");
        if (FlyaudioInit.isColorSkin!=null) {
            if (FlyaudioInit.isColorSkin.equals("isSystemUIColorSkin")) {
                isColorSkin = true;
            }else {
                isColorSkin = false;
            }
        }
        if (!colorFromPro.equals("-65536")&&isColorSkin) {
            mQsContainer.setBackgroundColor(Integer
                    .parseInt(colorFromPro));
            line.setBackgroundColor(Integer
                    .parseInt(colorFromPro));
            mlv.setColor(Integer
                    .parseInt(colorFromPro));
        } else {
            mQsContainer.setBackgroundColor(SkinResource.getSkinColorByName("m_qs_container_bg_color"));
            line.setBackgroundColor(SkinResource.getSkinColorByName("color_background_header_line"));
            //line.setBackground(SkinResource.getSkinDrawableByName("systemui_header_line"));
            mlv.setColor(SkinResource.getSkinColorByName("m_line_view_pointer_color"));
        }
    }

    /*初始化 开关通知 分割线*/
    public void initSkinFlyHeaderView() {
        //mQsContainer.setBackground();
        rb_default = Integer.parseInt(SkinResource.getSkinStringByName("status_bar_default_textsize"));
        rb_selected = Integer.parseInt(SkinResource.getSkinStringByName("status_bar__selected_textsize"));

        color_header_textview_d = SkinResource.getSkinColorByName("color_text_selected_textview");

        color_header_textview_u = SkinResource.getSkinColorByName("color_text_normal_textview");

        RelativeLayout.LayoutParams layoutParams =
                new RelativeLayout.LayoutParams(switchAndNotificationRoot.getLayoutParams());
        switch_notification_marginStart = (int) SkinResource.getSkinDimenFromSkin("status_bar_switch_notification_marginStart");
        layoutParams.setMarginStart(switch_notification_marginStart);
        switchAndNotificationRoot.setLayoutParams(layoutParams);

        switchView.setText(
                SkinResource.getSkinStringByName("fly_switch"));
        /*switchView.setBackground(
                SkinResource.getSkinDrawableByName("drawable_radiobutton_default"));*/
        switchView.setTextColor(color_header_textview_d);

        notificationView.setText(
                SkinResource.getSkinStringByName("fly_notifications"));
        /*notificationView.setBackground(
                SkinResource.getSkinDrawableByName("drawable_radiobutton_default"));*/
        notificationView.setTextColor(color_header_textview_u);

        if (SkinResource.getSkinStringByName("isShowOrView").equals("true")) {
            Integer or_view_width = SkinResource.getIntegerByName("or_view_width");
            if (or_view_width!=0) {
                or_tv.setWidth(or_view_width);
            }
            or_tv.setVisibility(View.VISIBLE);
            or_tv.setTextColor(SkinResource.getSkinColorByName("color_choose_state_textview_or"));
            or_tv.setText(SkinResource.getSkinStringByName("line_tag"));
            or_tv.setTextSize(SkinResource.getSkinDimenFromSkin("status_bar_default_textsize"));
        }else {
            or_tv.setVisibility(INVISIBLE);
        }

        //changeLineViewLayoutParams(true);
        line.setBackgroundColor(SkinResource.getSkinColorByName("color_background_header_line"));
        Integer status_bar_header_line_height = SkinResource.getIntegerByName("status_bar_header_line_height");
        if (status_bar_header_line_height!=null) {
            LayoutParams params=new LayoutParams (
                    LayoutParams.MATCH_PARENT,status_bar_header_line_height);
...
//        lp = (FrameLayout.LayoutParams) mNotificationStackScroller.getLayoutParams();
     // flyaudio notification 有卡时重新加载布局
     	LinearLayout.LayoutParams lp1 = (LinearLayout.LayoutParams) mNotificationStackScroller
     				.getLayoutParams();
        ...
        lp = (LayoutParams) mScrollView.getLayoutParams();
        if (lp.width != panelWidth) {
            lp.width = panelWidth;
            lp.gravity = panelGravity;
...
//                    mQsTracking = false;
    ...
    private boolean handleQsTouch(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_DOWN && getExpandedFraction() == 1f
        	...
        	//flyaudio panel 防止上滑时走进onQsTouch（）
			// mQsTracking = true;
			// end
...
//        }
        ...
        // flyaudio 两指触摸改为一指
 		if (mTwoFingerQsExpandPossible
 				&& event.getY(event.getActionIndex()) < mStatusBarMinHeight) {

 		   MetricsLogger.count(mContext, COUNTER_PANEL_OPEN_QS, 1);
           mQsExpandImmediate = true;//解决第一次下滑不能显示
           requestPanelHeightUpdate();
           //flyaudio
           mViewFlipper.setDisplayedChild(0);
           setRadioButtonTextSize(0);
		   //flyaudio
           // Normally, we start listening when the panel is expanded, but here we need to start
           // earlier so the state is already up to date when dragging down.
           setListening(true);
            ...
            case MotionEvent.ACTION_CANCEL:
             ...
             // flyaudio panel
    			...
    			/*if ((fraction != 0f || y >= mInitialTouchY)
    					&& (fraction != 1f || y <= mInitialTouchY)) {
    				Flog.d(TAG,
    						"onQsTouch===MotionEvent.ACTION_CANCEL===00000000000000");
    				// flingQsWithCurrentVelocity(event.getActionMasked() ==
    ...
    private void updateQsState() {
        ...
        if (mKeyguardShowing) {
            ...
            mNotificationStackScroller.setScrollingEnabled(false);
            mQsContainer.setVisibility(View.INVISIBLE);
            mScrollView.setTouchEnabled(false);
            mQsNavbarScrim.setVisibility(View.INVISIBLE);
        }
        //end
...
//        mNotificationStackScroller.setAlpha(alpha);
        if (alpha > 0) {
            mNotificationStackScroller.setBackground(
                    SkinResource.getSkinDrawableByName("systemui_tab2_background"));
            }else {
            mNotificationStackScroller.setBackgroundColor(Color.TRANSPARENT);
            }
        //flyaudio end
    }
...
//                (mNotificationStackScroller.getLeft() + mNotificationStackScroller.getWidth() / 2));
    ...
    /*开关/通知 字体大小*/
    int oldwhich=-1;
    private void setRadioButtonTextSize(int which){
        switch (which) {
            case 0:
                switchView.setTextSize(rb_selected);
                notificationView.setTextSize(rb_default);
                switchView.setTextColor(color_header_textview_d);
                notificationView.setTextColor(color_header_textview_u);
                if (oldwhich!=which) {
                    changeLineViewLayoutParams(true);
                ...
                break;
            case 1:
                switchView.setTextSize(rb_default);
                notificationView.setTextSize(rb_selected);
                switchView.setTextColor(color_header_textview_u);
                notificationView.setTextColor(color_header_textview_d);
                if (oldwhich!=which) {
                    changeLineViewLayoutParams(false);
                }
                break;
        }
        oldwhich = which;
    ...
    private void changeLineViewLayoutParams(boolean isState){
        ...
        if (isState) {
                layoutParams.setMarginStart(
                        (switchView.getMeasuredWidth()/2));
            } else {
                layoutParams.setMarginStart(switchView.getMeasuredWidth()+
                        or_tv.getMeasuredWidth()+(notificationView.getMeasuredWidth()/2)-9);
            }
        layoutParams.topMargin=81;
        mlv.setLayoutParams(layoutParams);
    }
}
